...
function InputOTPSlot({ char, hasFakeCaret, isActive, placeholderChar, className, ...props }: SlotProps & React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'relative flex h-10 w-8 min-w-8 sm:h-12 sm:w-10 sm:min-w-10 md:h-14 md:min-w-12 shrink-0 items-center justify-center rounded-md border border-gray-300 bg-white text-base sm:text-lg font-mono font-semibold tabular-nums transition-colors',
...
>(({ className, containerClassName, ...props }, ref) => (
  <OTPInput
    ...
    containerClassName={cn(
      'group flex items-center justify-center gap-x-1.5 sm:gap-x-2 has-[:disabled]:opacity-50 max-w-full overflow-hidden',
      'min-w-0 px-0.5 otp-input-root',
      containerClassName
    )}
